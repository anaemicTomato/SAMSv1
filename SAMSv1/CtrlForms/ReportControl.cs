...
namespace SAMSv1.CtrlForms
{
    public partial class ReportControl : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private void AttendanceLogAvian_Load(object sender, EventArgs e)
        {
            ...
            c.Properties.Client = gcStudentTable;
        ...
        {
            var raw = _repo.GetAttendance(
                GetSelectedDates(),
                GetSelectedEventId(),
                GetSelectedCourse(),
                GetSelectedYearLevel(),
                GetSelectedSession(),
                GetSelectedSemester()).ToList();

            // ── Insert blank separator between session groups ──────
            var display = new List<AttendanceReportRow>();
            string lastSession = null;

            foreach (var row in raw)
            {
                if (lastSession != null && row.Session != lastSession)
                {
                    // blank separator row
                    display.Add(new AttendanceReportRow
                    {
            ...
            gcStudentTable.DataSource = display;
        }
